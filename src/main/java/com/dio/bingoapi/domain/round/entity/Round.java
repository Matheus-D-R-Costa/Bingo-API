...
import static com.dio.bingoapi.domain.round.enums.RoundState.*;
import static com.dio.bingoapi.infrastructure.configuration.exception.BaseErrorMessage.ROUND_ALREADY_FINISHED;
import static com.dio.bingoapi.infrastructure.configuration.exception.BaseErrorMessage.ROUND_ALREADY_STARTED;

public record Round(String id,
    ...
    public static class RoundBuilder {
        ...
        public Mono<RoundBuilder> addBingoCard(Player player) {
            return Mono.defer(() -> {
                if (state != CREATED) return Mono.error(new RoundAlreadyStartedException(
                        ROUND_ALREADY_STARTED.params(this.id).getMessage()));

                return BingoCard.builder().generate(player.id(), this.bingoCards)
        ...
        public Mono<RoundBuilder> sortNumber() {
            return Mono.defer(() -> {
                if (state == FINISHED) return Mono.error(new RoundAlreadyFinishedException(
                        ROUND_ALREADY_FINISHED.params(this.id).getMessage()));

                if (state == CREATED) this.state = IN_PROGRESS;
        ...
        private Mono<RoundBuilder> processSortedNumber(Integer sortedNumber) {
        ...
        private Mono<RoundBuilder> finish() {
            return Mono.just(this.state(FINISHED).updatedAt(OffsetDateTime.now()));
        }
